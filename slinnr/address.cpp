...
template<class T>
static T CreateMask(uint32_t bites) {
    return (1 << (sizeof(T) * 8 - bites)) - 1;
}
...
bool Address::GetInterfaceAddresses(std::multimap<std::string, std::pair<Address::ptr, uint32_t>> & result, int family) {
    ...
    if (getifaddrs(&results) != 0) {
        SYLAR_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses getifaddrs err=" << errno <<" errstr" << strerror(errno);
        return false;
    }
    try {
