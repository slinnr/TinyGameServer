...
void ByteArray::write(const void* buf, size_t size) {
    ...
    while (size > 0) {
        if (ncap >= size) {
            ...
            memcpy(m_cur->prt + npos, (const char*)buf + bpos, ncap);
            m_position += ncap;
            bpos += ncap;
            size -= ncap;
            m_cur = m_cur->next;
...
void ByteArray::read(void* buf, size_t size) {
    ...
    while (size > 0)
    {
        if (ncap >= size) {
            ...
            memccpy((char*)buf + bpos, m_cur->ptr + npos, ncap);
            m_position += ncap;
            bpos += ncap;
            size -= ncap;
            m_cur = m_cur->next;
...
uint64_t ByteArray::getReadBuffers(std::vector<iovec>& buffers, uint64_t len) const {
    len = len > getReadSize() ? getReadBuffers() : len;
    if (len == 0) {
        return 0;
    }
    uint64_t size = len;
